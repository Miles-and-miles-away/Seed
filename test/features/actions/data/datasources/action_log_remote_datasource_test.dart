...
void main() {
  ...
  setUp(() {
    ...
    dataSource = ActionLogRemoteDataSource(firestore: fakeFirestore);
  });

  CollectionReference<Map<String, dynamic>> logsCollection(String uid) =>
      fakeFirestore
  ...
  group('ActionLogRemoteDataSource', () {
    group('watchUserActionLogs', () {
      test('returns stream ordered by loggedAt desc', () async {
        ...
        await seedLog(userId, 'log2', loggedAt: DateTime(2024, 6, 15));
        await seedLog(userId, 'log3', loggedAt: DateTime(2024, 6, 14));
    ...
    group('watchUserActionLogs limit', () {
      test('caps the stream at the requested number of logs', () async {
        for (var i = 0; i < 5; i++) {
          await seedLog(userId, 'log$i', loggedAt: DateTime(2024, 6, 10 + i));
        }
    ...
    group('range queries', () {
      test(
        'watchActionLogsForRange returns only logs in [start, end)',
        () async {
      ...
      test(
        'getActionLogsForRange returns empty when nothing matches',
        () async {
