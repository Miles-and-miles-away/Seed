...
void main() {
  group('EcoDexCondition.fromJson', () {
    test('totalActions', () {
      final c = EcoDexCondition.fromJson({'type': 'totalActions', 'count': 25});
    ...
    test('co2Saved', () {
      final c = EcoDexCondition.fromJson({'type': 'co2Saved', 'grams': 5000});
      expect(c, isA<Co2SavedCondition>());
      expect((c as Co2SavedCondition).grams, 5000);
    });

    test('streakDays', () {
      final c = EcoDexCondition.fromJson({'type': 'streakDays', 'days': 7});
      expect(c, isA<StreakDaysCondition>());
      expect((c as StreakDaysCondition).days, 7);
    });

    test('levelReached', () {
      final c = EcoDexCondition.fromJson({'type': 'levelReached', 'level': 15});
      expect(c, isA<LevelReachedCondition>());
      expect((c as LevelReachedCondition).level, 15);
    });

    test('sdgBreadth', () {
      final c = EcoDexCondition.fromJson({'type': 'sdgBreadth', 'count': 5});
    ...
    test('multiDayChallenge', () {
      ...
      expect((c as MultiDayChallengeCondition).templateId, 'vegan_week');
    });
    ...
    test('profileComplete', () {
      final c = EcoDexCondition.fromJson({'type': 'profileComplete'});
      expect(c, isA<ProfileCompleteCondition>());
    });

    test('ecodexCount', () {
      final c = EcoDexCondition.fromJson({'type': 'ecodexCount', 'count': 10});
