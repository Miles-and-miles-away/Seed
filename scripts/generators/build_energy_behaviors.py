...
Run from the repo root:  python3 scripts/generators/build_energy_behaviors.py
"""
import json

ACC = '2026-08-02'
S = lambda n, u, q: {'name': n, 'url': u, 'quote': q, 'accessed': ACC}

EPA = S('US EPA WaterSense (showerheads)',
...
HPTCJ = S('Heat Pump & Thermal Storage Technology Center of Japan',
...
WHICH = S('Which? UK (dishwasher vs hand washing, 03 Jul 2026)',
...
WHICH_DW = S('Which? UK (dishwasher main wash, 03 Jul 2026)',
             'https://www.which.co.uk/news/article/which-research-reveals-how-little-'
             'water-dishwashers-use-compared-to-hand-washing-aUDng9Y2iK8E',
...
BOSCH_WM_20 = S(_WM_N, _WM_U, 'Βαμβακερά (Cottons) 20 °C | 9,0 | 0,350 | 89,0 | 3:02')
BOSCH_WM_40 = S(_WM_N, _WM_U, 'Βαμβακερά (Cottons) 40 °C | 9,0 | 1,300 | 89,0 | 3:48')
BOSCH_WM_60 = S(_WM_N, _WM_U, 'Βαμβακερά (Cottons) 60 °C | 9,0 | 1,700 | 89,0 | 3:03')
BOSCH_DRY_C = S('Bosch WTM8327SZA condenser dryer, 8 kg',
...
BOSCH_DRY_HP = S('Bosch WQG24509GB heat-pump dryer, 9 kg',
...
BOSCH_DW = S('Bosch SMS67MW00G dishwasher, 14 place settings',
...
METI_AC_COOL = S('資源エネルギー庁 省エネポータルサイト (空調)',
...
METI_AC_HEAT = S('資源エネルギー庁 省エネポータルサイト (空調)',
    'https://www.enecho.meti.go.jp/category/saving_and_new/saving/general/howto/airconditioning/index.html',
    '暖房を1日1時間短縮した場合（設定温度：20℃）年間で電気40.73kWhの省エネ')
OCWR = S('Office of Congressional Workplace Rights (US)',
...
FRONTIER_IH = S('Frontier Energy, Residential Cooktop Performance and Energy '
...
FRONTIER_GAS = S('Frontier Energy, Residential Cooktop Performance and Energy '
                 'Comparison Study, Report #501318071-R0 (July 2019)',
                 'https://cao-94612.s3.amazonaws.com/documents/Induction-Range-Final-Report-July-2019.pdf',
...
EU_OVEN = S('Commission Regulation (EU) No 66/2014, Annex II '
...
EU_OVEN = {**EU_OVEN, 'accessed': '2026-08-29'}
RICE = S('一般財団法人 家電製品協会 (省エネルギーセンター実測値)',
...
IFIXIT = S('iFixit iPhone 15 battery', 'https://www.ifixit.com/products/iphone-15-battery',
           '12.98 Wh')
METI_TV = S('資源エネルギー庁 省エネポータルサイト (娯楽)',
...
LBNL = S('Lawrence Berkeley National Laboratory, Standby Power', 'https://standby.lbl.gov/',
...
SHOWER_PRESETS = [
    P('quick_5min', 'A quick shower (5 min)', 'さっとシャワー（5分）', 'Ducha rápida (5 min)', 5),
    P('typical_10min', 'A typical shower (10 min)', 'ふつうのシャワー（10分）', 'Ducha normal (10 min)', 10),
    P('long_20min', 'A long shower (20 min)', '長めのシャワー（20分）', 'Ducha larga (20 min)', 20),
]
BATH_PRESETS = [
    P('full_180l', 'A full bath (~180 L)', 'ふつうの湯量（約180L）', 'Baño normal (~180 L)', 1.0),
    P('shallow_150l', 'A shallow bath (~150 L)', '浅めの湯量（約150L）', 'Baño poco profundo (~150 L)', 0.83),
]
WASHUP_PRESETS = [
    P('full_load', 'A full load (14 place settings)', '食器14人分をまとめて', 'Carga completa (14 servicios)', 1.0),
    P('few_dishes', 'A few dishes', '少しの食器', 'Unos pocos platos', 0.25),
]
ONE_LOAD = [P('one_load', '1 load', '1回', '1 carga', 1)]
ONE_CYCLE_DW = [P('one_cycle', '1 cycle (14 place settings)', '1回（食器14人分）', '1 ciclo (14 servicios)', 1)]
BOIL_PRESETS = [
...
BEHAVIORS = [
 ...
 dict(id='bath_electric', group='hot_water', carrier='electricity', unit='use',
      ...
      sources=[SUIDO, SAP],
      notes='180 L x 0.03162756 kWh/L = 5.692960 kWh. The largest single-use figure in '
            'the dataset. RESEARCH_ENERGY section 3.1.'),
 ...
 dict(id='aircon_heating', group='space_heat', carrier='electricity', unit='hour',
      ...
      notes='METI publishes 40.73 kWh/year saved by shortening heating one hour a day '
            ...
            'CS-227VB JIS rating is 455 W, about 1.9x higher, because it is measured at '
            'full load. Do not "correct" this to the catalog figure. Setpoint presets '
            'use METI absolute deltas (+0.034898 kWh/h per 1 C) and are capped at +2 C '
            'because neither source supports extrapolating further. '
            'RESEARCH_ENERGY section 3.3.'),
 dict(id='portable_electric_heater', group='space_heat', carrier='electricity', unit='hour',
      kwh=1.2, en='Portable electric heater', ja='電気ストーブ／セラミックファンヒーター',
      es='Calefactor eléctrico portátil', conf='medium', presets=EVENING_4H, sources=[OCWR],
 ...
 dict(id='kotatsu', group='space_heat', carrier='electricity', unit='hour',
      kwh=0.15, en='Kotatsu (heated table)', ja='こたつ', es='Kotatsu (mesa con calefactor)',
 ...
 dict(id='oven', group='cook', carrier='electricity', unit='use',
      ...
      notes='PER BAKE CYCLE, not per hour, and that is structural rather than a gap: '
            ...
            'exists anywhere. Do not "fix" it to an hourly unit. From the EU formula at '
            '60-70 L cavities (0.802-0.844), shipping the 0.82 midpoint. Lowest '
            'confidence of the cooking entries: it is an EU-to-US proxy. '
            'RESEARCH_ENERGY section 3.4.'),
 ...
 dict(id='rice_keepwarm', group='cook', carrier='electricity', unit='hour',
      ...
      presets=[P('four_hours', '4 hours (the guideline)', '4時間（目安）', '4 horas (la guía)', 4),
               P('overnight_10h', 'Overnight (10 h)', '一晩（10時間）', 'Toda la noche (10 h)', 10)],
      sources=[RICE],
 ...
 dict(id='led_bulb', group='lighting', carrier='electricity', unit='hour',
      ...
      presets=ONE_HOUR + [P('evening_5h', 'An evening (5 h)', '夜に5時間', 'Una tarde (5 h)', 5)],
      sources=[],
 ...
 dict(id='incandescent_bulb', group='lighting', carrier='electricity', unit='hour',
      ...
      notes='NO CITATION SHIPS, same reason as the LED entry: no URL was captured '
            ...
            'LED, a 7.06x ratio. Japan never legally banned incandescents, so a blanket '
            '"phased out" claim is false for a JP reader. '
            'RESEARCH_ENERGY section 3.5.'),
 # ---- device ----
 dict(id='tv', group='device', carrier='electricity', unit='hour',
      ...
      presets=ONE_HOUR + [P('evening_3h', 'An evening (3 h)', '夜に3時間', 'Una tarde (3 h)', 3)],
      sources=[METI_TV],
 ...
 dict(id='laptop_charge', group='device', carrier='electricity', unit='use',
      ...
      presets=[P('one_charge', '1 full charge', '1回フル充電', '1 carga completa', 1)],
      sources=[],
      notes='A 53.8 Wh MacBook Air battery at 85% charging efficiency. Sourced from the '
            'manufacturer spec quoted verbatim in RESEARCH_ENERGY section 3.5, whose '
 ...
 dict(id='standby', group='device', carrier='electricity', unit='day',
      ...
      notes='25-40 always-on devices at about 1 W. Against a US home at ~29 kWh/day '
            ...
            'per-device standby collapsed from 1-3 W to about 0.5 W while device counts '
            'rose faster, leaving roughly the same household total spread over many '
            'more products. Not "standby is trivial" and not "standby is 10% of your '
            'bill". Excludes builder-installed always-on load (smoke alarms, '
...
out = {
    'metadata': {
        ...
        'scope': (
            ...
            'use, multiplied by a carrier factor. Generation-basis electricity with '
            'no well-to-tank term, and combustion-only gas, matching the transport '
            'dataset scope. Excludes appliance manufacturing, transmission and '
            'distribution losses, and water supply and treatment '
            'energy. One global grid factor for every user; per-country factors were '
            'considered and not shipped (decision E1). Whether a T&D correction is '
            'warranted is an open question rather than a settled exclusion '
            '(Plan/PDR_GRID_REGIONALISATION.md section 4.6). Nearly every comparison '
            'here is within one carrier, where the grid factor cancels out entirely. '
            'NEVER sum a figure from this calculator with one from the food or '
            'transport calculators: food counts a lifecycle, transport counts tailpipe '
            'energy, and this counts the electricity or gas to run a home.'
        ),
        'grid_factor_g_per_kwh': 458,
        'grid_factor_source': (
            'Ember, Global Electricity Review 2026 (2025 world average, CO2e). '
            'Shared with data/app/transport_modes.json and pinned by a cross-dataset '
            'test. Decision E1, 2026-08-02.'
        ),
        ...
        'awards_note': (
            ...
            'shower has no verifiable counterfactual, and the action library already '
            'covers the same behaviours. Decision 8.18, permanent, not deferred.'
        ),
    ...
    'behaviors': [
        {
            ...
            'confidence': b['conf'],
            'presets': b['presets'],
            'default_preset_id': b['presets'][0]['id'] if b['presets'] else '',
...
PATH = 'data/app/energy_behaviors.json'
with open(PATH, 'w', encoding='utf-8') as f:
    f.write(json.dumps(out, indent=2, ensure_ascii=False) + '\n')

import collections
g = collections.Counter(b['comparable_group'] for b in out['behaviors'])
print(f'wrote {PATH}: {len(out["behaviors"])} behaviors')
print('groups:', dict(g))
assert len(out['behaviors']) == 33, len(out['behaviors'])
ids = [b['id'] for b in out['behaviors']]
assert len(set(ids)) == 33, 'duplicate id'
