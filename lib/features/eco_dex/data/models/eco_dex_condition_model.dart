...
sealed class EcoDexCondition with _$EcoDexCondition {
  ...
  const factory EcoDexCondition.totalActions({required int count}) =
  ...
  const factory EcoDexCondition.co2Saved({required int grams}) =
      Co2SavedCondition;

  /// Unlock after maintaining a streak of N days.
  const factory EcoDexCondition.streakDays({required int days}) =
      StreakDaysCondition;

  /// Unlock after reaching user level N.
  const factory EcoDexCondition.levelReached({required int level}) =
      LevelReachedCondition;

  /// Unlock after logging actions related to N distinct SDGs.
  const factory EcoDexCondition.sdgBreadth({required int count}) =
      SdgBreadthCondition;

  /// Unlock after reaching a challenge streak of N days.
  const factory EcoDexCondition.challengeStreak({required int days}) =
  ...
  const factory EcoDexCondition.ecoFactsViewed({required int count}) =
      EcoFactsViewedCondition;

  /// Unlock after logging actions in N distinct categories.
  const factory EcoDexCondition.categoriesCovered({required int count}) =
      CategoriesCoveredCondition;

  /// Unlock after logging N unique action types.
  const factory EcoDexCondition.uniqueActionsLogged({required int count}) =
  ...
  const factory EcoDexCondition.challengesCompleted({required int count}) =
      ChallengesCompletedCondition;

  /// Unlock after logging N distinct zero-CO2 ("selfless") actions.
  const factory EcoDexCondition.uniqueZeroCo2Actions({required int count}) =
